...
def board_scene(kind):
    out=[]
    def text(x,y,s,size=18,c=INK):
        # Modest hierarchy: retain small labels, soften only the large callouts.
        out.append(('label',x,y,s,size*.94 if size>18 else size,c))
    ...
    def arrow(points,c=GRAY):
        ...
        for a,b in zip(points,points[1:]):line(a,b,c,1.3)
        a,b=points[-2:];theta=math.atan2(b[1]-a[1],b[0]-a[0])
        for delta in [-.45,.45]:line(b,(b[0]-9*math.cos(theta+delta),b[1]-9*math.sin(theta+delta)),c,1.3)
    ...
    if kind=='parallel-angles':
        ...
        text(331,37,'② こちらも同じ。',21,BROWN);text(331,65,'2本の弧が対応。',17,BROWN);text(331,94,'∠AED = ∠ACB',17,BROWN)
        ...
        line(a,d,BLUE,3.4);line(d,b,BROWN,3.4,'8 5');text(154,171,'3',25,BLUE);text(80,265,'2',25,BROWN)
        ...
        text(319,40,'合わせると',19);text(319,74,'全体は 3+2=5',23)
        x=480;line((x,114),(x,234),BLUE,3.4);line((x,234),(x,314),BROWN,3.4,'8 5')
