...
- 問29〜32は翌日以降の再挑戦。
- A4の解説18ページ・問題集18ページ・解答解説16ページ。問題集の最後の2ページは体験メモ。
...
- 問題集と同じ1ページ2問、全16ページ。ヒント・答え・全途中式・注意点は原稿の内容を保持。
- `scripts/similarity_answers.py` で問題用図を解答用に補い、左の図と右の途中式を並べる。
- 求めた値は青で図に記入。対応する辺・等しい角と、式中の記号を同色で結ぶ。実線／破線の下線も併用。
- 比の向きは対応表、条件不足は2辺の開き方の比較、座標の面積は共通の底辺OCと横の距離で図示。
- 図の文字9 pt以上、途中式10.5 pt。改行で句読点だけが次行に残らないよう調整。
- 全16ページを画像で確認。PDF内の全32問のヒント・答え・途中式・注意点が原稿と一致することを検証。相似DOM・ブラウザ検証も成功。
- 確認用固定名：`previews/similarity-answers-visual-v2.pdf`。
